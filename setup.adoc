...
For the implementation we are going to connect to a Amazon Elastic Compute Cloud (Amazon EC2) virtual machine.
...
NOTE: the `i-xxx...` string is the `instanceId`. Take note of it in order to be able to terminate the instance when you are done with it.
...
== Terminate the EC2 virtual machine

We ask you to terminate the virtual machine once you have completed your work. Type `exit` to close the `ssh` session and the following command to terminate the machine (use the `instanceId` you collected when you lanched the virtual machine):
